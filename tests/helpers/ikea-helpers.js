...
export async function addProductToCart(page, category = 'cat/sofas-fu003/') {
  ...
  try {
    await page.goto(categoryPath, { timeout: 15000, waitUntil: 'domcontentloaded' });
  } catch (e) {
    if (e.name === 'TimeoutError' || e.message.includes('timeout') || e.message.includes('navigation')) {
      test.skip(true, `Category page navigation timed out or was blocked by bot challenge: ${categoryPath}`);
    } else {
      throw e;
    }
  }
  ...
  if (!navigated || !page.url().includes('/p/')) {
    ...
    try {
      await page.goto(fallbackUrl, { timeout: 15000, waitUntil: 'domcontentloaded' });
    } catch (e) {
      if (e.name === 'TimeoutError' || e.message.includes('timeout') || e.message.includes('navigation')) {
        test.skip(true, `Fallback product page navigation timed out or was blocked by bot challenge: ${fallbackUrl}`);
      } else {
        throw e;
      }
    }
